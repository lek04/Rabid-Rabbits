    ...
    for (;;)
    {
        ...
        else if (playerMove.size() == 1)
        {
            if (tolower(playerMove[0]) == 'c')
                ...
                return player->move(dir);
            else if (tolower(playerMove[0]) == 'h') { //added this, to bring up the history grid
                m_arena->history().display();
                cout << "Press enter to continue.";//allowed since it's allowed in play() and the functions it calls
                cin.ignore(10000, '\n');
                m_arena->display("");
            }
        }
